    ...
    <div class="col-sm">
      <div class="btn-group ">
        <form action="/users/{{$member->id}}" method="POST">
          ...
          @if($member->admin)
          <button type="submit" title="Admin" class="btn-sm btn-warning me-1 my-1">Demote from Admin</button>
          @else
          <button type="submit" title="Admin" class="btn-sm btn-warning me-1 my-1 ">Promote to Admin</button>
          @endif
          @endcan
        </form>
        ...
        <form action="/users/{{$member->id}}" method="POST">
          ...
          <button type="submit" title="delete" class="mx-auto btn-sm btn-danger my-1">Delete Member</button>
          @endcan
        </form>
      </div>
    </div>
